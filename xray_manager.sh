...
EOFRENEW
    ...
    # Добавление в crontab (надежный способ через временный файл)
    tmp_cron=$(mktemp)
    crontab -l 2>/dev/null | grep -v "xray-cert-renew" > "$tmp_cron" || true
    echo "0 1 1 * * * /usr/local/etc/xray/xray_cert/xray-cert-renew $domain" >> "$tmp_cron"
    crontab "$tmp_cron"
    rm "$tmp_cron"
...
EOFUTIL
    ...
    tmp_cron=$(mktemp)
    crontab -l 2>/dev/null | grep -v "cleanup_expired" > "$tmp_cron" || true
    echo "0 * * * * /usr/local/bin/xray_manager.sh cleanup_expired" >> "$tmp_cron"
    crontab "$tmp_cron"
    rm "$tmp_cron"
