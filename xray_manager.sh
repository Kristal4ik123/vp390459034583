...
EOFRENEW
    ...
    # Добавление в crontab (максимально надежный способ без pipe)
    if ! crontab -l 2>/dev/null | grep -q "xray-cert-renew"; then
        crontab -l 2>/dev/null > /tmp/cron_tmp || true
        echo "0 1 1 * * * /usr/local/etc/xray/xray_cert/xray-cert-renew $domain" >> /tmp/cron_tmp
...
EOFUTIL
    ...
    if ! crontab -l 2>/dev/null | grep -q "cleanup_expired"; then
        crontab -l 2>/dev/null > /tmp/cron_tmp_2 || true
        echo "0 * * * * /usr/local/bin/xray_manager.sh cleanup_expired" >> /tmp/cron_tmp_2
        crontab /tmp/cron_tmp_2
        rm -f /tmp/cron_tmp_2
    fi
