...
EOFRENEW
    ...
    # Добавление в crontab (корректная обработка для чистых систем)
...
EOFUTIL
    ...
    (crontab -l 2>/dev/null | grep -v "cleanup_expired"; echo "0 * * * * /usr/local/bin/xray_manager.sh cleanup_expired") | crontab -
